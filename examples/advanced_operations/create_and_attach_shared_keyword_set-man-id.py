...
def main(client, customer_id, campaign_id, manager_id):
    ...
    shared_set.name = f"Manager API Negative keyword list - {uuid.uuid4()}"
    shared_set.type_ = client.enums.SharedSetTypeEnum.NEGATIVE_KEYWORDS

    response_content_type = client.enums.ResponseContentTypeEnum.MUTABLE_RESOURCE
    request = client.get_type("MutateSharedSetsRequest")
    request.operations = [shared_set_operation]
    request.customer_id = manager_id
    request.response_content_type = response_content_type
    try:
        shared_set_response = shared_set_service.mutate_shared_sets(
            request
        )
        shared_set_creation_result = shared_set_response.results[0]
        shared_set = shared_set_creation_result.shared_set
        shared_set_resource_name = shared_set_creation_result.resource_name
        shared_set_resource_id = shared_set.id

        logger.info(f'Created shared set "{shared_set_resource_name}".')
